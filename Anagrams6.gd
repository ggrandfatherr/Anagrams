...
func select_new_string():
	new_string = EngDict.six_dict.keys()[randi() % EngDict.six_dict.size()]
...
func check_word():
	if final_string.length() > 2:
		if !final_string in used_words and (EngDict.small_dict.has(final_string.to_lower()) or EngDict.six_dict.has(final_string.to_lower())):
			used_words.append(final_string)
			
			var blink = false
			if rt_score(final_string.length()) == 2000:
				blink = true
		...
		elif !EngDict.small_dict.has(final_string.to_lower()) or !EngDict.six_dict.has(final_string.to_lower()):
...
#		print("index: ",x_pos," - ",btn.id, " = ", x_pos - btn.id," btn_id ",btn.id)
		tile_tween.interpolate_property(btn,"rect_position",btn.rect_position,Vector2((x_pos - btn.id) * 43,-45),0.15)
		tile_tween.start()
		btn.rect_position = Vector2((x_pos - btn.id) * 43,-45)
		btn.moved = true
...
#			print(_btn," |",btn," of ",btn.id, " | " ,btn.x_pos," > ", _btn.x_pos, btn.x_pos > _btn.x_pos," at: ",btn.rect_position, " ||| ",new_x)
			return_tween.interpolate_property(btn,"rect_position",null,Vector2((btn.x_pos - btn.id - 2) * 43,-45),0.1)
			return_tween.start()
			btn.rect_position = Vector2((btn.x_pos - btn.id - 2) * 43,-45)
...
func sort_arr(arr:Array):
	...
	for word in arr:
		var new_string_len = len(word)
		match new_string_len:
			3:
				tre.append(word)
			4:
				fou.append(word)
...
#	var word_co = display_panel.instance()
#	word_co.word = "Words spelt: " + str(used_words.size())
#	full_stck.add_child(word_co)
...
func set_highscore():
	if score > EngDict.high_score_six:
		EngDict.set_six_highscore(score)
		hs_pane.visible = false
		s_pane.visible = true
		final_score.text = "New High Score: " + str(EngDict.get_six_highscore()).pad_zeros(4) + "!"
	else:
		hs_pane.visible = true
		final_score.text = "Score: " + str(score).pad_zeros(4)
		high_score.text = "HighScore: " + str(EngDict.get_six_highscore()).pad_zeros(4)
